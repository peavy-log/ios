...
extension Peavy {
    ...
    public static func logRequest(_ tracer: PeavyTracing, request: inout URLRequest) {
        let trace = logRequest(tracer, url: request.url, method: request.httpMethod)
    ...
    public static func logRequest(_ tracer: PeavyTracing, url: URL?, method: String?) -> PeavyTrace {
        let trace = tracer.newTrace()
        logRequest(trace, url: url, method: method)
        return trace
    }

    /// Logs an HTTP trace to Peavy
    public static func logRequest(_ trace: PeavyTrace, url: URL?, method: String?) {
